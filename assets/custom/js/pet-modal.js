$(document).ready(function () {
                    ...
                    <div class="col-lg-3 my-2 my-lg-0">
                        <div class="existing-pet p-2">
                            <div class="edit-delete text-end p-1 m-1">
                                ...
                                <i class="fa fa-trash delete-pet_popup" data-index="${data[i]['user_pety_id']}" aria-hidden="true" data-img-src="${data[i]['pet_img']}"></i>
                            </div>
                            <div class="pets-details-wrapper d-flex justify-content-between align-items-center">
                                <div class="pets-image">
                                    <img  src="${base_url}${data[i]['pet_img']}" alt="" class="img-fluid">
                                </div>
    ...
    // when date change automatically calculate age and show 
    $("#add-pet_popup #dob").on("change", function () {
        ...
        if (isNaN(dob.getTime())) {
            $("#add-pet_popup #age_year").val('');
            $("#add-pet_popup #age_month").val('');
            return;
        }
        ...
        $("#add-pet_popup #age_year").val(years);
        $("#add-pet_popup #age_month").val(months);
    });
    ...
    $("#add-pet").click(function () {
        ...
        else if ((userPetMode == "new") && $("#pet_img").val() == "") {


            $(".pet-img-error").text("Please upload image").show();
        }
        else {
    ...
    $(document).on('click', '.edit-pet_popup', function () {
        ...
        $("#old_img").val(pet['pet_img']);

        $("#pet_image_url").attr("src", base_url+pet['pet_img']).show();
...
$(document).on('click', '.delete-pet_popup', function () {
    const userPetID = $(this).data("index"); // more robust with `const`
        ...
        buttons: {
            confirm: function () {
                $.ajax({
                    ...
                    data: { user_pety_id: userPetID, pet_img: imgSrc}, // ensure the key matches what backend expects
                    success: function (response) {
                        getUserPetList();
                        TOASTER_HANDLER(response);
                    },
